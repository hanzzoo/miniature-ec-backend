...
class CartItemRepository:
    ...
    async def update_product_to_cart(
        self, instance: str, product_id: str, quantity: int
    ) -> None:
        try:
            ...
            query = select(CartItem).where(
                CartItem.instance == instance, CartItem.product_id == product_id
            )
            result = await self.db.execute(query)
    ...
    async def get_cart_items(self, instance: str) -> list[CartItem]:
        try:
            await self._validate_cart_instance(instance)

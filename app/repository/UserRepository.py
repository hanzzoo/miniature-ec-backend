...
from app.schemas import AuthorizeUserSchema, AuthorizeUserSchemaWrapper
...
class UserRepository:
    ...
    async def register_user(
        self, user_name: str, user_email: str, user_password: str
    ) -> str | None:
    ...
    async def authorize_user(
    ...
    ) -> tuple[bool, AuthorizeUserSchemaWrapper]:
        ...
        try:
            ...
            if not user:
                return False, AuthorizeUserSchemaWrapper(
                    user=AuthorizeUserSchema(user_id=None, user_name=None)
                )
            is_password = bcrypt.checkpw(
                user_password.encode(), user.user_password.encode()
            )
            return is_password, AuthorizeUserSchemaWrapper(
                user=AuthorizeUserSchema(
                    user_id=str(user.user_id), user_name=str(user.user_name)
                )
            )
        except Exception as e:
